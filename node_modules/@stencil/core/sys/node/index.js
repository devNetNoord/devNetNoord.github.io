/*!
 Stencil Node System v4.19.2 | MIT Licensed | https://stenciljs.com
 */
...
`;if(U[j]===vi)return`
`}});return function(U){return K.apply(this,arguments)}})(),I=t.writeFilePreservingEol=(()=>{var K=(0,(r||i()).default)(function*(G,U){let j=(yield T(G))||(O||m()).default.EOL;j!==`
...
`).map(function(g){return g.trim()}).join(" ")},t.formatters.O=function(x){return this.inspectOpts.colors=this.useColors,i.inspect(x,this.inspectOpts)};function a(x){var g=this.namespace,C=this.useColors;if(C){var T=this.color,I="\x1B[3"+(T<8?T:"8;5;"+T),R="  "+I+";1m"+g+" \x1B[0m";x[0]=R+x[0].split(`
`).join(`
`+R),x.push(I+"m+"+t.humanize(this.diff)+"\x1B[0m")}else x[0]=d()+g+" "+x[0]}function d(){return t.inspectOpts.hideDate?"":new Date().toISOString()+" "}function c(){return process.stderr.write(i.format.apply(i,arguments)+`
